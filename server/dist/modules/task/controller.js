...
import { createTaskService, deleteTaskService, getCategories, getSharedCategories, getSharedTasksService, getTaskByIdService, getTasksService, shareTaskService, updateTaskService, } from "./service.js";
export const createTask = asyncHandler(async (req, res) => {
    logger.info("create task request received", {
        userId: req.user?.userId,
    });
    const task = await createTaskService(req.user.userId, req.validated.body);
    logger.info("task created successfully", {
        taskId: task.id,
        userId: req.user?.userId,
    });
    return sendResponse(res, StatusCodes.CREATED, "Task created successfully", task);
});
export const getTasks = asyncHandler(async (req, res) => {
    logger.info("get tasks request received", {
        userId: req.user?.userId,
    });
    const result = await getTasksService(req.user.userId, req.validated.query);
    logger.info("tasks fetched successfully", {
        userId: req.user?.userId,
    });
    return sendResponse(res, StatusCodes.OK, "Tasks fetched successfully", result);
});
...
export const updateTask = asyncHandler(async (req, res) => {
    ...
    const { id } = req.validated.params;
    const task = await updateTaskService(req.user.userId, id, req.validated.body);
    if (!task) {
...
export const getTaskCategories = asyncHandler(async (req, res) => {
    ...
    return sendResponse(res, StatusCodes.OK, "Task categories fetched successfully", categories);
});
export const shareTask = asyncHandler(async (req, res) => {
    ...
    const { notification, sharedWithId } = await shareTaskService(ownerId, taskId, email);
    return sendResponse(res, StatusCodes.OK, "Task shared successfully", {
        notificationId: notification.id,
        sharedWithId,
    });
});
export const getSharedTaskCategories = asyncHandler(async (req, res) => {
    const categories = await getSharedCategories(req.user.userId);
    return sendResponse(res, StatusCodes.OK, "Shared task categories fetched successfully", categories);
});
export const getSharedTasks = asyncHandler(async (req, res) => {
    ...
    return sendResponse(res, StatusCodes.OK, "Shared tasks fetched successfully", result);
});
