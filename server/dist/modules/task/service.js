...
export async function createTaskService(ownerId, data) {
...
export async function getTasksService(ownerId, filters) {
    ...
    const tasksWithOverdue = tasks.map((task) => ({
        ...task,
        isOverdue: task.status !== "DONE" && !!task.dueDate && new Date(task.dueDate) < now,
    }));
    return {
...
export async function getTaskByIdService(ownerId, taskId) {
...
export async function updateTaskService(ownerId, taskId, data) {
...
export async function deleteTaskService(ownerId, taskId) {
    const task = await prisma.task.findFirst({
        where: {
            id: taskId,
            ownerId,
...
export async function getCategories(ownerId) {
...
export async function shareTaskService(ownerId, taskId, email) {
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (sharedWith.id === ownerId) {
            throw new ApiError(StatusCodes.BAD_REQUEST, "You cannot share task with yourself");
        }
        ...
        if (existingShare) {
            throw new ApiError(StatusCodes.BAD_REQUEST, "Task already shared with this user");
        }
        // create share record
...
export async function getSharedTasksService(userId, filters) {
    const { status, priority, category, search, page, limit } = filters;
    const taskWhere = {
        deletedAt: null,
    };
    ...
    const tasks = sharedTasks.map((share) => ({
        ...
        isOverdue: share.task.status !== "DONE" &&
            !!share.task.dueDate &&
...
export async function getSharedCategories(userId) {
    const categories = await prisma.task.findMany({
        where: {
            deletedAt: null,
