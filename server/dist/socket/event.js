...
export function emitNotification(userId, notification) {
    try {
        ...
        logger.error(`Socket emit failed: ${err instanceof Error ? err.message : String(err)}`);
    }
}
// emit task shared event to specific user
export function emitTaskShared(userId, payload) {
    try {
        const io = getIO();
        ...
        logger.error(`Socket task shared emit failed: ${err instanceof Error ? err.message : String(err)}`);
    }
}
